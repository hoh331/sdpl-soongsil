...
class CreateClass(CreateClass_Super):        
    def tab_setup(self):
        ...
        self.TTZC_Operation_Pulse_ParamBox = ui.CreateParameterSettingsBox('Pulse setup', 9)   
        ...
        self.TTZC_Operation_Pulse_ParamBox.set_item(7, 'Delay before sweep starts', '1', 's')
        self.TTZC_Operation_Pulse_ParamBox.set_item(8, 'Measurement time step', '50', 'ms')
    ...
    def run_measurement_start(self):      
        ...
        self.wait_time = round(float(self.TTZC_Operation_Pulse_ParamBox.le_list[7].text()), ROUNDNUM)
        self.time_step = round(float(self.TTZC_Operation_Pulse_ParamBox.le_list[8].text()), ROUNDNUM)
...
class IO_Thread(IO_Thread_Super):  
    ...
    def run(self):
            ...
            if self.flag == 1:
                ...
                if CURRENT_TIME > self.stress_time_list:
                    ...
                    if self.is_pulse_rightnow(CURRENT_TIME, self.pulse_time_list, self.pulse_width):          
                        ...
                        self.SMU_WT_GATE.write(":SOUR:VOLT:LEV ", str(wt_gate_bias_amp))
                        ...
                        temp = np.array([CURRENT_TIME, rt_drain_bias, CURRENT_RT_DRAIN, 
                                         ...
                                         wt_gate_bias_amp, CURRENT_WT_GATE])
                        ...
                        self.SMU_RT_DRAIN.write(":SOUR:VOLT:LEV ", str(rt_drain_bias))
                        self.SMU_WT_DRAIN.write(":SOUR:VOLT:LEV ", str(0))
                        self.SMU_WT_GATE.write(":OUTP ON")
                        self.SMU_RT_DRAIN.write(":OUTP ON")
                        self.SMU_WT_DRAIN.write(":OUTP ON")
        ...
        wt_drain_bias = self.wt_drain_bias_list[0]
        wt_gate_bias = self.wt_gate_bias_list[0]        
