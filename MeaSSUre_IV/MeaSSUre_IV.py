...
import time
import numpy as np
import Measure_TFT_UI_all as ui
...
class App(QWidget):       
    def __init__(self):
        ...
        self.RunBox.btn_abort.clicked.connect(self.stop_measurement)
    ...
    def setup_tabs(self):
        ...
        """Output Tab Setup"""
        self.Output_Drain_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Drain bias setup', 8, 2)
        ...
        self.Output_Gate_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Gate bias setup', 7, 2)
        ...
        self.Output_GraphBox.set_titles(1, 'Gate leakage curves (Igs-Vgs)', 'Vds (V)', 'Igs (A)')
        ...
        self.Transfer_Gate_ParamBox =  ui.CreateParameterSettingsBoxWithToggle('Gate bias setup', 8, 2)
        ...
        self.Transfer_Drain_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Drain bias setup', 7, 2)
        ...
        self.IV_Drain_ParamBox =  ui.CreateParameterSettingsBoxWithToggle('Drain bias setup', 9, 2)
        ...
        self.IV_Drain_ParamBox.set_item(8, 'Drain NPLC', '0.1', 'number')
        ...
        """Collector Tab Setup"""
        self.Collector_Drain_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Collector bias setup', 8, 2)
        ...
        self.Collector_Drain_ParamBox.set_item(0, 'Vce start', '0', 'V')
        self.Collector_Drain_ParamBox.set_item(1, 'Vce stop', '10', 'V')
        self.Collector_Drain_ParamBox.set_item(2, 'Vce step', '0.1', 'V')
        ...
        self.Collector_Drain_ParamBox.set_item(5, 'Drain trigger delay', '0.1', 's')
        self.Collector_Drain_ParamBox.set_item(6, 'Drain source delay', '0.1', 's')
        self.Collector_Drain_ParamBox.set_item(7, 'Drain NPLC', '0.1', 'number')
        
        self.Collector_Gate_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Base current setup', 7, 2)
        ...
        self.Collector_Gate_ParamBox.set_item(0, 'Ibe (start)', '5, 10, 20, 30', 'uA')
        self.Collector_Gate_ParamBox.set_item(1, 'Ibe stop', '10', 'uA')
        self.Collector_Gate_ParamBox.set_item(2, 'Ibe step', '10', 'uA')
        self.Collector_Gate_ParamBox.set_item(3, 'Measurement settings', '0', '0')
        self.Collector_Gate_ParamBox.show_only_name(3)
        self.Collector_Gate_ParamBox.set_item(4, 'Gate trigger delay', '0.1', 's')
        self.Collector_Gate_ParamBox.set_item(5, 'Gate source delay', '0.1', 's')
        self.Collector_Gate_ParamBox.set_item(6, 'Gate NPLC', '0.1', 'number')
        
        self.Collector_GraphBox = ui.CreateGraphBox('Collector characteristics', 1)
        self.Collector_GraphBox.set_titles(0, 'Collector characteristic curves (Ice-Vce)', 'Vce (V)', 'Ice (A)')
        ...
        if self.scan_type == "Output":
            ...
            if array[0] == 9999:
                ...
                if np.isnan(array)[0] == True:
                    ...
                    self.LiveBox.set_status_run()
                    self.LiveBox.set_values(['%.4e%s' %(array[2], 'V'), '%.4e%s' %(array[3], 'A'), '%.4e%s' %(array[1], 'V'), '%.4e%s' %(array[0], 'A')])
    ...
    def run_measurement(self):
        ...
        if self.scan_type == "Output":
            ...
            print ("Preparing bias parameters")
            self.SMU_init_params = [[],[]]
            self.SMU_init_params[0].append(round(float(self.Transfer_Drain_ParamBox.le_list[-3].text()), ROUNDNUM))
            self.SMU_init_params[0].append(round(float(self.Transfer_Drain_ParamBox.le_list[-2].text()), ROUNDNUM))
            self.SMU_init_params[0].append(round(float(self.Transfer_Drain_ParamBox.le_list[-1].text()), ROUNDNUM))
            
            self.SMU_init_params[1].append(round(float(self.Transfer_Gate_ParamBox.le_list[-3].text()), ROUNDNUM))
            self.SMU_init_params[1].append(round(float(self.Transfer_Gate_ParamBox.le_list[-2].text()), ROUNDNUM))
            self.SMU_init_params[1].append(round(float(self.Transfer_Gate_ParamBox.le_list[-1].text()), ROUNDNUM))
            print ("Bias parameters:")
            print (self.SMU_init_params[0])
            print (self.SMU_init_params[1])
...
class IO_Thread(QtCore.QThread):
    ...
    def init_SMU(self, SMU, Param_list):
        ...
        SMU.write(":SENS:CURR:PROT MAX")        
        SMU.write(":SENS:CURR:NPLC ", str(Param_list[0]))
        SMU.write(":TRIG:DEL ", str(Param_list[1]))
        SMU.write(":SOUR:DEL ", str(Param_list[2]))
    ...
    def init_SMU_Curr(self, SMU, Param_list):
        ...
        SMU.write(":SOUR:CURR:RANG 100E-6")
        SMU.write(":SENS:FUNC \"VOLT\"")
        SMU.write(":SENS:VOLT:PROT 25")  
        SMU.write(":SENS:VOLT:RANG 20")
        SMU.write(":SENS:VOLT:NPLC ", str(Param_list[0]))
        SMU.write(":TRIG:DEL ", str(Param_list[1]))
        SMU.write(":SOUR:DEL ", str(Param_list[2]))
    ...
    def run(self):
        if self.scan_type == "Output":
            ...
            for vgs in self.vgs_list:
                ...
                print("Vgs value:")
                print(vgs_str)
                self.SMU_GATE.write(":SOUR:CURR:LEV ", vgs_str)
                self.SMU_GATE.write(":OUTP ON")
                self.signal.emit(new_curve) #Notify main function to draw a new curve
                time.sleep(self.wait_time)
                for vds in self.vds_list:
