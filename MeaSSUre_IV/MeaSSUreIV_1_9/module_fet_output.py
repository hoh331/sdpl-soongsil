...
class CreateClass(CreateClass_Super):        
    def tab_setup(self):
        ...
        self.Output_SecondGate_ParamBox = ui.CreateParameterSettingsBoxWithToggle('Second Gate bias setup', 8, 2)
        self.Output_SecondGate_ParamBox.set_toggle_item(0, 'Linear steps', False)
        self.Output_SecondGate_ParamBox.set_toggle_item(1, 'Custom steps', True)
        self.Output_SecondGate_ParamBox.set_item(0, 'Vgs (start)', '5, 10, 20, 30', 'V')
        self.Output_SecondGate_ParamBox.set_item(1, 'Vgs stop', '10', 'V')
        self.Output_SecondGate_ParamBox.set_item(2, 'Vgs step', '10', 'V')
        self.Output_SecondGate_ParamBox.set_item(3, '<Measurement settings>', '0', '0')
        self.Output_SecondGate_ParamBox.show_only_name(3)
        self.Output_SecondGate_ParamBox.set_item(4, 'Gate trigger delay', '0.01', 's')
        self.Output_SecondGate_ParamBox.set_item(5, 'Gate source delay', '0.01', 's')
        self.Output_SecondGate_ParamBox.set_item(6, 'Gate NPLC', '1', 'number')
        self.Output_SecondGate_ParamBox.set_item(7, 'Current compliance', '100E-3', 'A')
        self.Output_SecondGate_ParamBox.groupbox.setCheckable(True)
        self.Output_SecondGate_ParamBox.groupbox.setChecked(False)
        ...
        self.tabOutput_vbox.addWidget(self.Output_SecondGate_ParamBox.groupbox)
    ...
    def update_plot(self, array):
        ...
        if array[0] == 99999999:
            ...
            if np.isnan(array)[0] == True:
                ...
                self.result_data[self.N] = array # save the current emitted data to entire record
    ...
    def run_measurement_start(self):      
        ...
        # calculate list for the Second gate biases
        if self.Output_SecondGate_ParamBox.groupbox.isChecked():            
            if self.Output_SecondGate_ParamBox.rbtn_list[1].isChecked():
                text = self.Output_SecondGate_ParamBox.le_list[0].text()
                self.vgs2_list = [float(x) for x in text.split(',')]
                
            else:                
                vgs2_start = round(float(self.Output_SecondGate_ParamBox.le_list[0].text()), ROUNDNUM)
                vgs2_stop =  round(float(self.Output_SecondGate_ParamBox.le_list[1].text()), ROUNDNUM)
                vgs2_step =  round(float(self.Output_SecondGate_ParamBox.le_list[2].text()), ROUNDNUM)     
                
                temp = vgs2_start
                while (temp <= vgs2_stop):
                    self.vgs2_list.append(temp)
                    temp = round((temp + vgs2_step), ROUNDNUM)   

            self.tot_num_measure_points = len(self.vds_list)*len(self.vgs_list)*len(self.vgs2_list)
        ...
        if self.Output_SecondGate_ParamBox.groupbox.isChecked():   
            self.SMU_init_params[2].append(round(float(self.Output_SecondGate_ParamBox.le_list[-4].text()), ROUNDNUM))
            self.SMU_init_params[2].append(round(float(self.Output_SecondGate_ParamBox.le_list[-3].text()), ROUNDNUM))
            self.SMU_init_params[2].append(round(float(self.Output_SecondGate_ParamBox.le_list[-2].text()), ROUNDNUM))
            self.SMU_init_params[2].append(round(float(self.Output_SecondGate_ParamBox.le_list[-1].text()), ROUNDNUM))
            print (self.SMU_init_params[2])
                
        print ("Prepare measurement...")
        # Prepare data array for save
        if self.Output_SecondGate_ParamBox.groupbox.isChecked():   
            self.result_data = np.empty((self.tot_num_measure_points, 9))
        else:
...
class IO_Thread(IO_Thread_Super):        
    def run(self):
        ...
        self.vgs2_list = self.kwargs.get('vgs2_list')
        
        if len(self.vgs2_list) != 0:    
            self.SMU_SECONDGATE = self.SMU_list[2]
        
        new_curve = np.empty(6)
        new_curve[:] = np.NaN
        
        if len(self.vgs2_list) == 0: # Conventional case
        ...
        else: # When second gate is used
            for vgs2 in self.vgs2_list:
                if self.flag == 1:
                    self.SMU_SECONDGATE.write(":SOUR:VOLT:LEV ", str(vgs2))
                    self.SMU_SECONDGATE.write(":OUTP ON")
                else:
                    break;
                ...
                for vgs in self.vgs_list:
                    ...
                    for vds in self.vds_list:
                        if self.flag == 1:
                            ...
                            if vds == self.vds_list[0]:
                                time.sleep(self.wait_time)
                            CURRENT_SECONDGATE = self.SMU_SECONDGATE.query_ascii_values(":READ?")
                            CURRENT_GATE = self.SMU_GATE.query_ascii_values(":READ?")
                            CURRENT_DRAIN = self.SMU_DRAIN.query_ascii_values(":READ?")
                            CURRENT_SECONDGATE = CURRENT_SECONDGATE[1]
                            CURRENT_GATE = CURRENT_GATE[1]
                            CURRENT_DRAIN = CURRENT_DRAIN[1]
        
                            temp = np.array([vgs, vds, CURRENT_GATE, CURRENT_DRAIN, np.log10(np.abs(CURRENT_GATE)), np.log10(np.abs(CURRENT_DRAIN)), vgs2, CURRENT_SECONDGATE, np.log10(np.abs(CURRENT_SECONDGATE))])
    ...
    def stop(self):  
        if len(self.vgs2_list) == 0:
            ...
            self.signal.emit([99999999,99999999,99999999,99999999,99999999,99999999,99999999,99999999,99999999])  

            print("IO_Thread stop called") 
            self.SMU_DRAIN.write("OUTP OFF")
            self.SMU_GATE.write("OUTP OFF")
            self.SMU_SECONDGATE.write("OUTP OFF")
