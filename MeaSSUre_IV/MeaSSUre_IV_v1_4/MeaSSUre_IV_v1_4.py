...
import time
import numpy as np
import MeaSSUre_IV_UI_all_v1_4 as ui
import os
...
class App(QWidget):       
    def __init__(self):
        ...
        grid.addWidget(self.DataSettingsBox.groupbox, 0,3,1,8)   
        grid.addWidget(self.TimeDisplayBox.groupbox, 0,11,1,2)
    ...
    def setup_tabs(self):
        ...
        self.tabs.addTab(self.tabPBSNBS, 'FET: PBS/NBS')
        self.tabs.addTab(self.tabIV, 'I-V')
        self.tabs.addTab(self.tabCollector, 'BJT: Collector C/C')
        self.tabs.addTab(self.tabMemory_Endurance, 'Memory Endurance')
        self.tabs.addTab(self.tabMemory_Retention, 'Memory Retention')
        self.tabtotalnum = 7
        ...
        self.IV_Drain_ParamBox =  ui.CreateParameterSettingsBoxWithToggle('Drain bias setup', 10, 3)
        ...
        self.IV_Drain_ParamBox.set_item(0, 'Vds start (Neg. Limit for cycle sweep)', '0', 'V')
        self.IV_Drain_ParamBox.set_item(1, 'Vds stop (Pos. Limit for cycle sweep)', '1', 'V')
        ...
        self.Memory_Endurance_ParamBox = ui.CreateParameterSettingsBox('Memory Endurance setup', 10)
        self.Memory_Endurance_ParamBox.set_item(0, 'Program gate bias', '-10', 'V')
        self.Memory_Endurance_ParamBox.set_item(1, 'Erase gate bias', '10', 'V')
        self.Memory_Endurance_ParamBox.set_item(2, 'Read gate bias', '0', 'V')
        self.Memory_Endurance_ParamBox.set_item(3, 'Read drain bias', '0.1', 'V')
        self.Memory_Endurance_ParamBox.set_item(4, 'Program/Erase Pulse width', '100', 'ms')
        self.Memory_Endurance_ParamBox.set_item(5, 'Delay beween squences', '0.5', 's')
        self.Memory_Endurance_ParamBox.set_item(6, 'Total number of cycles', '100', '#')
        self.Memory_Endurance_ParamBox.set_item(7, '<Measurement settings>', '0', '0')
        self.Memory_Endurance_ParamBox.show_only_name(7)
        self.Memory_Endurance_ParamBox.set_item(8, 'Drain NPLC', '1', 'number')
        self.Memory_Endurance_ParamBox.set_item(9, 'Current compliance', '1E-3', 'A')
        ...
        self.tabMemory_Endurance_vbox.addWidget(self.Memory_Endurance_ParamBox.groupbox)
    ...
    def update_plot(self, array):
        ...
        if self.scan_type == "Output":
        ...
        elif self.scan_type == "PBSNBS":
            print(array)
            if array[0] == 99999999:
                self.measurement_done_flag = True
                self.stop_measurement()
            elif array[0] == 99999998:
                self.LogBox.update_log("Applying bias stress for %d seconds" %(array[1]))
                self.LogBox.update_log("Remaining time: %d" %(array[1]))
            elif array[0] == 99999997:
                self.LogBox.remove_last_sentence()
                self.LogBox.update_log("Remaining time: %d" %(array[1]))
            else:
                if np.isnan(array)[0] == True:
            ...
            if array[0] == 99999999:
                ...
                if np.isnan(array)[0] == True:
                    ...
                    self.LiveBox.set_status_run()
                    self.LiveBox.set_values(['%.4e%s' %(array[0], 'V'), '%.4e%s' %(array[1], 'A'), '- V', ' -A'])
        ...
        elif self.scan_type == "Memory_Endurance":          
            print(array)
            if array[0] == 99999999:
                self.measurement_done_flag = True
                self.stop_measurement()
                ...
                if np.isnan(array)[0] == True:
                    self.Memory_Endurance_GraphBox.addnew_plot(0)
                    self.Memory_Endurance_GraphBox.addnew_plot(0)
                    self.count = 1
                    self.start = self.N
                    ...
                    self.LiveBox.set_status_run()
                    self.LiveBox.set_values(['%.4e%s' %(array[0], 'V'), '%.4e%s' %(array[1], 'A'), '%.4e%s' %(array[2], 'V'), '%.4e%s' %(array[3], 'uA')])
                    
        elif self.scan_type == "Memory_Retention":          
            print(array)
            if array[0] == 99999999:
                self.measurement_done_flag = True
                self.stop_measurement()
    ...
    def run_measurement(self):
        ...
        self.TimeDisplayBox.check_start_time()
        self.measurement_timer.timeout.connect(self.TimeDisplayBox.update_time_elapsed)
        ...
        if self.scan_type == "Output":
            ...
            vgs_program = round(float(self.Memory_Endurance_ParamBox.le_list[0].text()), ROUNDNUM)
            vgs_erase =  round(float(self.Memory_Endurance_ParamBox.le_list[1].text()), ROUNDNUM)
            vgs_read =  round(float(self.Memory_Endurance_ParamBox.le_list[2].text()), ROUNDNUM)
            vds_read =  round(float(self.Memory_Endurance_ParamBox.le_list[3].text()), ROUNDNUM)
            pulse_width_temp = float(self.Memory_Endurance_ParamBox.le_list[4].text())
            pulse_width = round(float(pulse_width_temp/1000), ROUNDNUM)
            wait_time = round(float(self.Memory_Endurance_ParamBox.le_list[5].text()), ROUNDNUM)
            repeat_num = round(int(self.Memory_Endurance_ParamBox.le_list[6].text()), ROUNDNUM)
            ...
            self.SMU_init_params[0].append(round(float(self.Memory_Endurance_ParamBox.le_list[-2].text()), ROUNDNUM))
            self.SMU_init_params[0].append(round(float(self.Memory_Endurance_ParamBox.le_list[-1].text()), ROUNDNUM))
            ...
            self.SMU_init_params[1].append(round(float(self.Memory_Endurance_ParamBox.le_list[-1].text()), ROUNDNUM))
    ...
    def tab_change(self, i):
        if i == 0:
            ...
            self.scan_type = "PBSNBS"
            self.RunBox.btn_run.setText("Run (FET: PBS/NBS)")
            self.ConnectionControlBox.lbl_list[0].setText("SMU1 (Drain)")
            self.ConnectionControlBox.lbl_list[1].setText("SMU2 (Gate)")
        elif i == 3:
            self.scan_type = "I-V"
            ...
            self.scan_type = "Memory_Endurance"
            self.RunBox.btn_run.setText("Run (Endurance)")
            self.ConnectionControlBox.lbl_list[0].setText("SMU1 (Drain)")
            self.ConnectionControlBox.lbl_list[1].setText("SMU2 (Gate)")
        else:
            self.scan_type = "Memory_Retention"
            self.RunBox.btn_run.setText("Run (Retention)")
            self.ConnectionControlBox.lbl_list[0].setText("SMU1 (Drain)")
            self.ConnectionControlBox.lbl_list[1].setText("SMU2 (Gate)")
...
class IO_Thread(QtCore.QThread):
    ...
    def run(self):
        if self.scan_type == "Output":
            ...
            for i in range(self.repeat_num):
                if self.flag == 1:
                    time.sleep(self.wait_time-self.OFFSET_TIME)
                    # Programming
            ...
            for i in range(self.repeat_num):
                if self.flag == 1:                                       
                    ...
                    temp = np.array([i, float(i*self.wait_time), temp_reading[5], temp_reading[1], np.log10(np.abs(temp_program[1]))])
